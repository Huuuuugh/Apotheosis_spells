...
/** 冷却时间 -N%（冷却乘算 = 1 - N/100） */
public class CooldownAffix extends SpellAffix {
    public static final Codec<CooldownAffix> C = RecordCodecBuilder.create(i -> i.group(
    ...
    ).apply(i, CooldownAffix::new));

    public CooldownAffix(String m, Map<String, Fn> v, Set<String> t) { super(m, v, t, AffixType.STAT); }

    @Override
    public ReforgeCache.Data contribute(int baseValue) {
        return new ReforgeCache.Data(1, 1, Math.max(0.05f, 1f - baseValue / 100f), 1, 0, 1, 1, 0);
    }

...
/** 施法疾步：每次施法后获得 N 级迅捷（短暂，事件类，SpellOnCastEvent 结算）。 */
public class SwiftnessAffix extends SpellAffix {
    public static final Codec<SwiftnessAffix> C = RecordCodecBuilder.create(i -> i.group(
    ...
    ).apply(i, SwiftnessAffix::new));

    public SwiftnessAffix(String m, Map<String, Fn> v, Set<String> t) { super(m, v, t, AffixType.STAT); }

    @Override
    public ReforgeCache.Data contribute(int baseValue) { return ReforgeCache.Data.DEF; }

    @Override
    public SpellEffects contributeEffect(int baseValue) { return SpellEffects.ofHaste(baseValue); }
